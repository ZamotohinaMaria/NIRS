...
if __name__ == '__main__':
    ...
    if cfg.run_model.lower() != 'relgan':
        raise ValueError("Only 'relgan' is supported in this cleaned repository.")
    if not cfg.if_real_data:
        raise ValueError("Only real-data mode is supported. Set --if_real_data 1.")

    from instructor.real_data.relgan_instructor import RelGANInstructor
    inst = RelGANInstructor(opt)
    if not cfg.if_test:
        inst._run()
    else:
        inst._test()
