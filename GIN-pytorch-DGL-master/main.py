...
def train(args):
    ...
    # load dataset
    train_set, test_set = loadNCI.load_nci_data(cuda=is_cuda)
    ...
    for epoch in range(args.num_epochs):
        ...
        print('MalBehavD: Epoch{}, loss {:.4f}, TrainACC {:.4f}, TestACC {:.4f}'.format(epoch, epoch_loss,
                                                                                      train_acc, test_acc))
        # print('Task {}: Epoch{}, loss {:.4f}, TrainACC {:.4f}'.format(nci_id, epoch, epoch_loss, train_acc))
        epoch_losses.append(epoch_loss)
    os.makedirs(MODEL_PATH, exist_ok=True)
    torch.save(model.state_dict(), MODEL_PATH + 'Model-MalBehavD.kpl')
    test_acc = utils.val(model, test_data_loader, is_cuda)
...
def main():
    ...
    # parameters
    args = parse_arg()
    train_acc, test_acc = train(args)
    print("\nResult:")
    print("MalBehavD: Train ACC: {:.4f}, Test ACC {:.4f}".format(train_acc, test_acc))
