...
DATA_PATH = './data/NCI_balanced/nci_malbehavd.pkl'
...
def load_nci_data(cuda=False):
    if not os.path.exists(DATA_PATH):
        raise FileNotFoundError(f"Dataset file not found: {DATA_PATH}")
    train_test_ratio = 0.7

    with open(DATA_PATH, 'rb') as nci_pkl_file:
    ...
    print(f'SUCCESS: Load dataset finished from {DATA_PATH}.\n')
...
def GetOneHotMap(element_list):
    ...
    #  binary encoder
    onehot_encoded = OneHotEncoder(sparse_output=False)
