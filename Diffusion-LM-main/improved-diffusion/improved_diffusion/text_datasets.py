    ...
    *, data_dir, batch_size, image_size, class_cond=False, deterministic=False, data_args=None, 
    ...
    if task_mode == 'roc' or task_mode == 'roc-aug' :
        ...
        training_data, model = get_corpus_rocstory(data_args, model, image_size,
                                            padding_mode=padding_mode, split=split,
                                            load_vocab=load_vocab)
    elif task_mode == 'malbehav':
        print('hello loading malbehav ')
        training_data, model = get_corpus_rocstory(data_args, model, image_size,
                                            padding_mode=padding_mode, split=split,
                                            load_vocab=load_vocab)
        ...
        if data_args.modality == 'roc-aug':
            ...
            with open(data_args.wiki_train, 'r') as ff:
                for row in ff:
                    ...
                    sentence_lst.append(word_lst)
            print(sentence_lst[:2])
        elif data_args.modality == 'malbehav':
            print('loading dataset from MalbehavD-V1')
            sentence_lst = []
            if split == 'train':
            ...
            if path == "":
                raise ValueError(f"empty path for malbehav split={split}")
            path = os.path.expanduser(path)
            if not os.path.exists(path):
                raise FileNotFoundError(f"malbehav dataset file not found: {path}")

            if path.endswith('.csv'):
                skip_cols = {
                    x.strip() for x in data_args.malbehav_skip_columns.split(',')
                    if x.strip() != ""
                }
                include_label = (data_args.malbehav_include_label == 'yes')
                label_col = data_args.malbehav_label_column
                with open(path, 'r', newline='', encoding='utf-8') as csvfile:
                    ...
                    for row in csv_reader:
                        ...
                        if include_label:
                            ...
                            if label_val != '':
                                token_lst.append(f'LABEL_{label_val}')
                        for col in fieldnames:
                            ...
                            if cell == '' or cell.lower() == 'nan':
                                continue
                            token_lst.append(cell)
                        if len(token_lst) > 0:
                            sentence_lst.append(token_lst)
            else:
                with open(path, 'r', encoding='utf-8') as ff:
                    for row in ff:
                        token_lst = [x for x in row.strip().split() if x != ""]
                        if len(token_lst) > 0:
                            sentence_lst.append(token_lst)

            print(sentence_lst[:2], 'dataset size=', len(sentence_lst))
